...

{
  ...
  # The ipmi_tower_* pair binds onto scripts/ipmi-remote.sh the same way, and
  ...
  #
  # ⚠ A raw IP rather than a name because the BMC has no DNS entry:
  # `PLATFORM.md` §2 records only `192.168.8.191`. If one ever gets added to
  # AdGuard, change it in both places (here and hosts/pegasus/home.nix).
  #
  ...
  home.shellAliases = {
    ...
    ipmi_tower_open_tty = ''~/Code/nixos-config/scripts/ipmi-remote.sh console 192.168.8.191 "op://System Keys/tower ipmi/password"'';
    ipmi_tower_set_bios_next_boot = ''~/Code/nixos-config/scripts/ipmi-remote.sh bios-next-boot 192.168.8.191 "op://System Keys/tower ipmi/password"'';
  };

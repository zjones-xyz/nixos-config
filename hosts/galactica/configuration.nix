...

{
  ...
  # or cert is expected for these. homeassistant deliberately has no `.xyz`
  # name at all — stays Tailscale/LAN-only. The *arr stack's `.xyz` route was
  # dropped entirely (owner's call: not needed).
  #
  # jellyfin and guesthome are NOT here — fleet/services.nix generates their
  # three rows (jellyfin.zjones.dev/.xyz, guesthome.zjones.xyz) and dns.nix
  # appends them to this list. See fleet/services.nix for the split-horizon/
  # flat-name reasoning those rows used to carry as inline comments here.
  ...
  services.adguardhome.settings.filtering.rewrites = map (r: r // { enabled = true; }) [
    ...
    # jellyfin.zjones.dev / jellyfin.zjones.xyz: generated by fleet/services.nix.
    ...
    { domain = "*.arr.zjones.dev"; answer = "192.168.8.190"; }
    # guesthome.zjones.xyz: generated by fleet/services.nix.
  ];

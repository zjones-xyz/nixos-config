...
Preferred: `colmena apply --on hopper` (or `--on @fleet` for the whole hive)
from pegasus — see `colmena.nix` at the repo root. It does the same
`--build-host`-style delegation to pegasus automatically via

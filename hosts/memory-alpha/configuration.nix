  ...
  boot.initrd.network = {
    ...
    ssh = {
      ...
      authorizedKeys = [
        ...
        # pegasus's outbound key — lets pegasus unlock memory-alpha's LUKS
        # root remotely (e.g. after a reboot) without going through serenity.
        # Same key already granted normal-system `z` access in
        # modules/nixos/common.nix.

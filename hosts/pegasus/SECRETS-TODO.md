...
- [ ] After the first boot, get pegasus's age key from its SSH host key:
      `ssh-to-age < /etc/ssh/ssh_host_ed25519_key.pub`
- [ ] In `.sops.yaml`, replace the `pegasus` placeholder key with that value and
      add `*pegasus` to the `secrets/pegasus.yaml` creation rule (a rule +
      placeholder are already stubbed in — see below).
- [ ] Create the encrypted file from the Mac (admin age key):
...
- [ ] `sops updatekeys secrets/pegasus.yaml`, commit, deploy. Until then, run
      `tailscale up --ssh` once interactively on the box.

## Borgmatic offsite backup (hosts/pegasus/borgmatic.nix)

- [ ] Generate a dedicated ed25519 keypair — do NOT reuse Tower's borgmatic
      key or z's own SSH key:
      ```sh
      ssh-keygen -t ed25519 -f pegasus-borgmatic -N ""
      ```
- [ ] Create the `pegasus-home` repo on BorgBase (see
...
- [ ] `sops secrets/pegasus.yaml` and add:
      ...
      ```
- [ ] `sops updatekeys secrets/pegasus.yaml`, commit, deploy.
- [ ] On pegasus, once deployed:
      `ssh-keyscan <borgbase-host> >> /var/lib/borgmatic/ssh/known_hosts`
      (not secret — doesn't go through sops).
- [ ] Turn on BorgBase's own inactivity alerting for this repo in its UI
      (docs/BACKUP.md §6) — that's the monitoring signal this config relies
      on; no ntfy/Kuma hook is wired for it.
- [ ] Run the first backup by hand and time it before trusting the systemd

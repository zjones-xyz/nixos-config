...
# creates the BorgBase repo and the two sops secrets it references. Until then
# the timer fires on schedule and fails loudly against the REPLACE repository
# URL; that is expected, not a bug (same stance as hosts/pegasus/borgmatic.nix).
#
...
in
{
  options.homelab.borgmatic = {
    enable = lib.mkEnableOption ''
      ...
      ZFS user property. Gate this on secret availability at the call site
      (e.g. `= hasSops`) so a host without secrets still evaluates and activates
    '';
    ...
    ntfy = {
      topic = lib.mkOption {
        type = lib.types.str;
        default = "galactica-backup";
        description = "ntfy topic to publish backup-failure alerts to.";
      };
      server = lib.mkOption {
        type = lib.types.str;
        default = "https://ntfy.hopper.internal";
        description = ''
          ntfy server. Defaults to the instance hopper runs
          (modules/nixos/ntfy.nix). ⚠ If that instance is locked down
          (`auth-default-access = "deny-all"`), set a token/credentials on the
          ntfy hook before this can publish — see BACKUP-BORG.md.
        '';
      };
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    services.borgmatic = {
      ...
      configurations.offsite = {
        ...
        # ── Failure alerting → ntfy (the fleet's existing channel) ────────────
        # `states = [ "fail" ]` so only failures notify — a headless backup that
        # silently stops is the failure mode worth a push. This complements, and
        # does not replace, turning on BorgBase's own inactivity alerting
        # (docs/BACKUP.md §3, §3b) as a heartbeat that does not depend on hopper.
        ntfy = {
          topic = cfg.ntfy.topic;
          server = cfg.ntfy.server;
          states = [ "fail" ];
          fail = {
            title = "galactica offsite backup FAILED";
            message = "borgmatic did not complete. Check `journalctl -u borgmatic` on galactica.";
            priority = "urgent";
